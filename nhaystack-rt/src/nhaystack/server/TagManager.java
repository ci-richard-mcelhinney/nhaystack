            ...
            for (Tag tag : tags)
            {
                ...
                if (tag.getId().getDictionary().equals(namespaces.get(i)))
                {
                    ...
                    else if (tagValueType == BString.TYPE)
                    {
                        ...
                        {
                            // this handles the bug introduced when using the h4 tag dictionary
                            // the new implementation of the implied Id tag from that dictionary
                            // returns an incompatible space with nhaystack
                            // specifically it returns an Ord using the new 'nspace' scheme
                            // nhaystack doesn't contemplate or otherwise recognise this space
                            // the bandaid solution is to just replace the invalid parts of the 
                            // Id tag that is returned above with a valid scheme
                            // in future this should be raised with Tridium to see if a native fix
                            // is available
                            if (value.contains("nspace") && value.contains("~slot:"))
                            {
