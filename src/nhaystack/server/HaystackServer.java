...
/**
  * HaystackServer is responsible for serving up 
  * haystack-annotated BComponents.
  */
public class HaystackServer extends HServer
{
...
////////////////////////////////////////////////////////////////
    ...
    protected Iterator iterator()
    {
        ...
        return new HaystackServerIterator(
    ...
    {
        // ASSUMPTION: the tz in both ends of the range matches the tz of the 
        // historized point, which in turn matches the history's tz in its
        // historyConfig.
        ...
        // NOTE: be careful, timeQuery() is inclusive of the both start and end
...
////////////////////////////////////////////////////////////////
    ...
    private void addHistoryTags(BHistoryConfig cfg, HDict tags, HDictBuilder hdb)
    {
        // add ax-specific tags
        hdb.add("axHistoryId", cfg.getId().toString());
    ...
    /**
      * add the 'kind' tag, along with an associated tags such as 'enum' or 'units'
      */
    protected void addPointKindTags(int pointKind, BFacets facets, HDict tags, HDictBuilder hdb)
    {
        switch(pointKind)
        {
            case NUMERIC_KIND:
    ...
    /**
      * TODO explain how IDs are generated for different spaces.
      */
    public HRef makeId(BComponent comp)
    {
        // history space
        if (comp instanceof BHistoryConfig)
        {
            ...
            return HRef.make(
                ...
                "h." + UriUtil.encodeToUri(hid.toString()));
        }
        // component space
        else
        {
    ...
    /**
      ...
      * or if it is not haystack-annotated.
      */
    public BComponent lookupById(HRef id)
    {
    ...
    {
        String[] a = TextUtil.split(id.val, ':');
        if (a.length != 2) return null;

        String[] b = TextUtil.split(a[1], '.');
        ...
        if (!station.equals(Sys.getStation().getStationName()))
            return null;

        // component space
        if (space.equals("c"))
        {
            BComponent comp = service.getComponentSpace().findByHandle(handle);
        ...
        {
            BHistoryId hid = BHistoryId.make(UriUtil.decodeFromUri(handle));
